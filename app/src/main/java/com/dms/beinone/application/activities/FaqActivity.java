...
public class FaqActivity extends AppCompatActivity {
    ...
    public void getFaq_detail(String id){
        ...
        service.loadNotice_detail(id).enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                ...
                switch (response.code()) {
                    case HTTP_OK:
                        Toast.makeText(FaqActivity.this, response.body().toString(), Toast.LENGTH_LONG).show();
                        Notice notice=getParserFaq(response.body().toString());
                        setNotice(notice);
                        break;
                    case HTTP_NO_CONTENT:
                        Toast.makeText(FaqActivity.this, "해당아이디 정보 없음", Toast.LENGTH_SHORT).show();
                        break;
                    default:
                        break;
                }
